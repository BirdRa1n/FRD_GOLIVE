...
- A v1 (SFU LiveKit) continua válida para **grupos grandes** e fica como modo
  opcional ("transporte: mesh | sfu"). O servidor pode oferecer os dois.
- O plugin abstrai o transporte atrás de uma interface (`RtcTransport`) para
  suportar `mesh` e `sfu`.
